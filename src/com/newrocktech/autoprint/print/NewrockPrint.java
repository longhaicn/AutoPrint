...
public class NewrockPrint {
	...
	public static List<LabelEntity> sendToPrint(String str, List<LabelEntity> lists, int printStyle, int updateStyle,
			String deletBIN) {
		String lbl[] = str.split(",");// workid+","+label_sort+","+sn1234+","+product_name+","+template_id;
		char workID = lbl[0].charAt(0);
		if (!lists.isEmpty()) {
			int i = 1;
			...
			String CURENT = "";
			if (lbl[1].contains("单直流")) {
				CURENT = "-1DC";
			} else if (lbl[1].contains("双直流")) {
				CURENT = "-2DC";
			} else if (lbl[1].contains("双交流")) {
				CURENT = "-2AC";
			}
			...
			if (1 == printStyle) {
				for (LabelEntity entity : lists) {
					...
					map.put("Model", entity.Model + CURENT);
		...
		if (0 == updateStyle) {// Clear log file from AutoUpdLogPath
			ZipUtils.deleteFile(ConfigUtils.get(R.AutoUpdLogPath), workID, deletBIN);
		}
		return lists;
	}
